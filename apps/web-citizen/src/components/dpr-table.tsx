...
export default function DprTable({ allocationCeiling = 350000, stateShare, csrShare, initialRows }: DprTableProps) {
  ...
  return (
    <section aria-label="Bill of Materials calculator" className="rounded-xl border border-[#F1F5F9] bg-white p-4 shadow-sm">
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-[#F1F5F9]">
                <td className="px-2 py-2">
                  <input value={r.component} onChange={(e) => update(r.id, { component: e.target.value })} placeholder="e.g. SS vessel 200L" aria-label="Component" className="w-full rounded-md border border-[#F1F5F9] px-2 py-1.5 text-sm outline-none focus:border-[#2563EB]" />
                </td>
                <td className="px-2 py-2">
                  <input type="number" min={0} value={r.qty} onChange={(e) => update(r.id, { qty: Number(e.target.value) })} aria-label="Quantity" className="w-20 rounded-md border border-[#F1F5F9] px-2 py-1.5 text-sm outline-none focus:border-[#2563EB]" />
                </td>
                <td className="px-2 py-2">
                  <input type="number" min={0} value={r.unitCost} onChange={(e) => update(r.id, { unitCost: Number(e.target.value) })} aria-label="Unit cost" className="w-28 rounded-md border border-[#F1F5F9] px-2 py-1.5 text-sm outline-none focus:border-[#2563EB]" />
                </td>
                <td className="px-2 py-2">
                  <input value={r.vendor} onChange={(e) => update(r.id, { vendor: e.target.value })} placeholder="Vendor" aria-label="Vendor" className="w-full rounded-md border border-[#F1F5F9] px-2 py-1.5 text-sm outline-none focus:border-[#2563EB]" />
                </td>
                <td className="px-2 py-2 text-right font-semibold tabular-nums">{formatINR(bomRowTotal(r))}</td>
                <td className="px-2 py-2 text-right">
                  <button onClick={() => remove(r.id)} aria-label="Remove row" className="rounded-md p-1.5 text-slate-500 hover:bg-blue-50 hover:text-blue-700">
      ...
      <button onClick={add} className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-[#F1F5F9] bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-800 hover:bg-slate-100">
      ...
      <div className="mt-4 rounded-lg bg-slate-50 p-3">
        <div className="h-2 overflow-hidden rounded-full bg-slate-200" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label="Budget utilisation">
          <div className={over ? "h-full bg-blue-500" : "h-full bg-[#1E3A8A]"} style={{ width: `${pct}%` }} />
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          <p className="font-bold tabular-nums">Total: {formatINR(total)}</p>
          <p className={over ? "text-xs font-semibold text-blue-700" : "text-xs font-semibold text-blue-800"}>
            {over ? `Over ceiling by ${formatINR(total - allocationCeiling)}` : `${formatINR(remaining)} remaining`}
          </p>
          <span className="ml-auto" />
          {over ? (
            <p role="alert" className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2.5 py-1 text-[11px] font-semibold text-blue-800">
