...
export function JuryScoreCard({ ticketId, compact = false }: { ticketId: string; compact?: boolean }) {
  ...
  return (
    <section
      ...
      className={`rounded-xl border-2 border-sky-300 bg-gradient-to-b from-sky-50 to-white p-4 shadow-sm ${compact ? "" : "sm:p-5"}`}
    >
      <div className="flex flex-wrap items-center gap-2">
        <p className="inline-flex items-center gap-1.5 rounded-full bg-[#0F172A] px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-sky-300">
          <Trophy className="h-3.5 w-3.5" /> Jury Presentation Mode · {ticketId}
        </p>
        <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-sky-100 px-2.5 py-1 text-[11px] font-bold text-sky-900">
          <Scale className="h-3.5 w-3.5" /> Feasibility 40% | Sustainability 30% | Cost 30%
        </span>
      </div>
      <div className="mt-3 grid gap-3 lg:grid-cols-3">
        {JURY_CRITERIA.map((c) => (
          <div key={c.id} className="rounded-lg border border-sky-200 bg-white p-3">
            <p className="text-sm font-bold text-slate-900">
              {c.label} <span className="text-xs font-semibold text-sky-700">({c.weight}%)</span>
            </p>
            ...
            <div className="mt-2 flex items-center gap-1" role="radiogroup" aria-label={`${c.label} star rating`}>
              {[1, 2, 3, 4, 5].map((s) => (
                <button
                  ...
                  onClick={() => set(c.id, s)}
                  className="rounded p-0.5 transition hover:scale-110 focus-visible:outline-2 focus-visible:outline-[#2563EB]"
                >
                  <Star
                    className={`h-6 w-6 ${s <= scores[c.id] ? "fill-sky-400 text-sky-500" : "fill-slate-100 text-slate-300"}`}
                  />
                </button>
              ))}
      ...
      {submitted && (
        <p role="status" className="mt-2 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800">
          {submitted}
        </p>
      )}
    </section>
