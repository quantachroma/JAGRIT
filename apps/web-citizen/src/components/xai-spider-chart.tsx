...
export default function XaiSpiderChart({ data, studentPool = 82, ticketId }: { data: XAISpiderChartData; studentPool?: number; ticketId?: string }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      <div className="rounded-xl border border-[#F1F5F9] bg-white p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">AI Capability Fit: {data.overallMatchScore}%{ticketId ? ` · ${ticketId}` : ""}</p>
          ...
          <div className="h-[260px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={chart} outerRadius="68%">
                ...
                <Radar name="Campus fit" dataKey="value" stroke="#2563EB" fill="#3B82F6" fillOpacity={0.35} strokeWidth={2} dot />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-4">
        <p className="inline-flex items-center gap-1.5 rounded-full bg-blue-900 px-2.5 py-1 text-[11px] font-semibold text-white"><Microscope className="h-3 w-3" /> Explainability card</p>
