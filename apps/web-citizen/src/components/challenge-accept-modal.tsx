    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 p-4 sm:items-center" role="dialog" aria-modal="true" aria-label="Challenge acceptance">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <button onClick={() => setMode("SOLO")} className={`rounded-xl border p-3 text-left ${mode === "SOLO" ? "border-[#2563EB] bg-blue-50" : "border-[#F1F5F9]"}`}>
          ...
          <button onClick={() => setMode("CONSORTIUM")} className={`rounded-xl border p-3 text-left ${mode === "CONSORTIUM" ? "border-[#1E3A8A] bg-blue-50" : "border-[#F1F5F9]"}`}>
        ...
        {mode === "CONSORTIUM" && (
          <label className="mt-3 block"><span className="text-xs font-semibold">Consortium partner</span>
            <select value={partner} onChange={(e) => setPartner(e.target.value)} className="mt-1 w-full rounded-lg border border-[#F1F5F9] px-3 py-2 text-sm">
        ...
        <div className="mt-4 rounded-xl bg-slate-50 p-3">
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <input value={piName} onChange={(e) => setPiName(e.target.value)} placeholder="PI name" className="rounded-lg border border-[#F1F5F9] bg-white px-3 py-2 text-sm" />
            <input value={piCode} onChange={(e) => setPiCode(e.target.value)} placeholder="BIT/EMP/2014-042" className="rounded-lg border border-[#F1F5F9] bg-white px-3 py-2 text-sm" />
          </div>
        </div>
        <div className="mt-3">
          ...
          <div className="mt-2 space-y-2">
            {students.map((s, i) => (
              <div key={i} className="grid gap-2 rounded-lg border border-[#F1F5F9] p-2 sm:grid-cols-[1fr_1fr_1fr_auto]">
                <input value={s.name} onChange={(e) => upd(i, "name", e.target.value)} placeholder="Student name" className="rounded-lg border border-[#F1F5F9] px-2.5 py-1.5 text-sm" />
                <input value={s.dept} onChange={(e) => upd(i, "dept", e.target.value)} placeholder="Department" className="rounded-lg border border-[#F1F5F9] px-2.5 py-1.5 text-sm" />
                <input value={s.apaar} onChange={(e) => upd(i, "apaar", e.target.value)} placeholder="APAAR / ID" className="rounded-lg border border-[#F1F5F9] px-2.5 py-1.5 text-sm" />
                <button onClick={() => setStudents(students.filter((_, j) => j !== i))} aria-label="Remove" className="rounded-lg p-1.5 text-slate-400 hover:bg-blue-50 hover:text-blue-600"><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
        </div>
