    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-[#F1F5F9] bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-2">
          {SLOTS.map((s) => (
            <button key={s} onClick={() => { setSlot(s); onBook(s); }} aria-pressed={slot === s} className={`rounded-lg border px-3 py-2 text-left text-sm ${slot === s ? "border-[#1E3A8A] bg-blue-50 font-bold" : "border-[#F1F5F9] hover:bg-slate-50"}`}>
              {s}
            </button>
          ))}
        </div>
        {slot && <p role="status" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-800"><CheckCircle2 className="h-3.5 w-3.5" />Booked: {slot}</p>}
      </div>
    </div>
  );
}
