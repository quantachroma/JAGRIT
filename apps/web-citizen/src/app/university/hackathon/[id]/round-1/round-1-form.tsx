...
export default function Round1Form({ ticketId, onToast }: { ticketId: string; onToast: (m: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-[#F1F5F9] bg-white p-4 shadow-sm">
      ...
      {fileName && !fileErr && <p role="status" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-800"><CheckCircle2 className="h-3.5 w-3.5" />{fileName}</p>}
      {fileErr && <p role="alert" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-700"><AlertTriangle className="h-3.5 w-3.5" />{fileErr}</p>}
      <div className="mt-4">
        ...
        <input id="r1-video" value={video} onChange={(e) => setVideo(e.target.value)} placeholder="https://loom.com/share/… or https://youtu.be/…" className="mt-2 w-full rounded-lg border border-[#F1F5F9] px-3 py-2 text-sm outline-none focus:border-[#2563EB]" />
        {video && !videoOk && <p role="alert" className="mt-1 text-xs font-semibold text-blue-700">Enter a valid YouTube / youtu.be / Loom / Vimeo link.</p>}
      ...
      <div className="mt-4">
        ...
        <textarea id="r1-narrative" value={narrative} onChange={(e) => setNarrative(e.target.value)} rows={4} placeholder="Problem, local insight, method, 14-day test plan…" className="mt-2 w-full rounded-lg border border-[#F1F5F9] px-3 py-2 text-sm outline-none focus:border-[#2563EB]" />
      </div>
      <button disabled={!ready} onClick={() => onToast(`Round 1 locked for ${ticketId}. Deck + video queued for shortlisting.`)} className="mt-4 w-full rounded-lg bg-[#1E3A8A] px-4 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300">
