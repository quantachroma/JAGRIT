  ...
  return (
    <div className="rounded-xl border border-[#F1F5F9] bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 grid gap-2">
        {DEFENSE_SLOTS.map((s) => (
          <button key={s} onClick={() => { setSlot(s); onBook(s); }} aria-pressed={slot === s} className={`flex items-center gap-1.5 rounded-lg border px-3 py-2 text-left text-sm ${slot === s ? "border-[#1E3A8A] bg-blue-50 font-bold" : "border-[#F1F5F9] hover:bg-slate-50"}`}>
            <CalendarCheck className="h-4 w-4 shrink-0" />{s}
          </button>
        ))}
      </div>
      {slot && <p role="status" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-800"><CheckCircle2 className="h-3.5 w-3.5" />Defense booked: {slot}</p>}
    </div>
  );
}
