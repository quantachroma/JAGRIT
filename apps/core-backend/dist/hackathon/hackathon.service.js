...
async function placeBid(challengeId, universityName, budget, facultyId) {
    const result = await (0, client_1.query)(`INSERT INTO public.projects
			(challenge_id, lead_university_name, total_budget_inr, pi_faculty_id, execution_mode)
		 VALUES ($1, $2, $3, $4, 'DIRECT_RND')
		 RETURNING *;`, [challengeId, universityName, budget, facultyId]);
    return result.rows[0];
}
async function evaluateBids(challengeId) {
    const countResult = await (0, client_1.query)('SELECT COUNT(*)::text AS bids_count FROM public.projects WHERE challenge_id = $1;', [challengeId]);
    const bidsCount = Number(countResult.rows[0].bids_count);
    if (bidsCount === 1) {
        await (0, client_1.query)("UPDATE public.challenges SET status = 'DIRECT_RND' WHERE id = $1;", [challengeId]);
        await (0, client_1.query)('UPDATE public.projects SET tranche_1_disbursed = TRUE WHERE challenge_id = $1;', [challengeId]);
        return {
            status: 'DIRECT_RND',
    ...
    if (bidsCount >= 2) {
        await (0, client_1.query)("UPDATE public.challenges SET status = 'DYNAMIC_HACKATHON' WHERE id = $1;", [challengeId]);
        await (0, client_1.query)(`UPDATE public.projects
