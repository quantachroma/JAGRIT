...
exports.createOrDeduplicateChallenge = createOrDeduplicateChallenge;
const client_1 = require("../db/client");
async function createOrDeduplicateChallenge(data) {
    const nearbyResult = await (0, client_1.query)(`SELECT id, ticket_number, upvotes_count,
		 ...
		 ORDER BY dist_meters ASC LIMIT 1;`, [data.lon, data.lat]);
    const nearbyChallenge = nearbyResult.rows[0];
    if (nearbyChallenge && Number(nearbyChallenge.dist_meters) <= 500) {
        const updatedResult = await (0, client_1.query)('UPDATE public.challenges SET upvotes_count = upvotes_count + 1 WHERE id = $1 RETURNING upvotes_count;', [nearbyChallenge.id]);
        return {
            is_duplicate: true,
    ...
    await (0, client_1.query)(`INSERT INTO public.challenges
