    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 p-4 sm:items-center" role="dialog" aria-modal="true" aria-label="Tripartite IPR concordat">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#4F46E5]">{ticket} · PRD 10.2 Concordat</p>
            <h2 className="flex items-center gap-2 text-lg font-bold"><ShieldCheck className="h-5 w-5 text-[#044728]" />Tripartite IPR Agreement</h2>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Dr. R. K. Verma" className="mt-1 w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-sm" />
        <label className="mt-3 flex cursor-pointer items-start gap-2 rounded-xl bg-slate-50 p-3 text-xs">
          <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="mt-0.5 h-4 w-4 accent-[#044728]" />
        ...
        {done && <p role="status" className="mt-2 flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800"><BadgeCheck className="h-4 w-4" />{done}</p>}
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <button onClick={download} disabled={!agreed} className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg px-4 py-2.5 text-sm font-semibold text-white ${agreed ? "bg-[#044728] hover:bg-emerald-900" : "cursor-not-allowed bg-slate-300"}`}>
            <Download className="h-4 w-4" />Accept IPR Terms & Download Agreement
          </button>
          <button onClick={onClose} className="rounded-lg border border-[#E2E8F0] px-4 py-2.5 text-sm font-semibold"><FileCheck2 className="mr-1 inline h-4 w-4" />Close</button>
        </div>
      </div>
    </div>
  );
