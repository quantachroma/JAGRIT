  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 p-4 sm:items-center" role="dialog" aria-modal="true" aria-label="Tripartite IPR concordat">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 sm:p-6">
        ...
        {done && <p role="status" className="mt-2 flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800"><BadgeCheck className="h-4 w-4 text-[#2563EB]" />{done}</p>}
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <button onClick={download} disabled={!agreed} className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg px-4 py-2.5 text-sm font-semibold text-white ${agreed ? "bg-[#1E3A8A] hover:bg-[#2563EB]" : "cursor-not-allowed bg-slate-300"}`}>
