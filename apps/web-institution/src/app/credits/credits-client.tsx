      ...
      <div className="rounded-xl border bg-white p-5">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl bg-slate-50 p-4">
            <p className="flex items-center gap-1.5 text-xs font-bold uppercase text-slate-500"><Calculator className="h-3.5 w-3.5 text-[#2563EB]" />Credit calculator</p>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {PRESETS.map((v) => (
                <button key={v} onClick={() => p.setHours(v)} className={`rounded-full px-3 py-1.5 text-xs font-semibold ${p.hours === v ? "bg-[#1E3A8A] text-white" : "bg-white text-slate-700 border hover:bg-slate-100"}`}>{v} hrs</button>
              ))}
            </div>
            <div className="mt-3 rounded-xl bg-[#0F172A] p-4 text-white">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-700"><div className="h-full bg-[#2563EB]" style={{ width: `${calc.credits >= 4 ? 100 : (calc.remainder / 30) * 100}%` }} /></div>
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-col gap-2">
        <button onClick={p.exportPayload} className="flex items-center justify-center gap-1.5 rounded-lg bg-[#1E3A8A] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#2563EB]"><Download className="h-4 w-4" />Generate Signed APAAR/ABC JSON Payload</button>
      ...
      {p.toast && <p role="status" className="mt-2 rounded-lg bg-blue-50 border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-800">{p.toast}</p>}
      <details className="mt-3 rounded-xl border bg-slate-950 p-3 text-[11px] text-sky-200">
