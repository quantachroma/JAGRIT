...
const INITIAL_ACCEPTED_PROJECTS: AcceptedProject[] = [
  {
    ...
    link: "/hackathon/JAG-4102/round-1",
  },
];
  ...
  const handleAccepted = (s: ChallengeStatus, m: string) => {
    if (acceptFor) {
      const newProj: AcceptedProject = {
        ...
        activeSprint: "Stage: Bench Prototyping & Lab Testing (Week 3/16)",
        facultyPI: "Dr. Anand Verma (BIT Mesra)",
        link: `/hackathon/${acceptFor.ticketId}/round-1`,
      };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-5">
        ...
        <h1 className="mt-1 text-2xl font-black text-slate-900">जनसमस्याएं एवं समाधान / Local Issues & Solutions</h1>
        ...
        {list.map((c) => (
          <article key={c.ticketId} className="flex flex-col rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs">
              <span className="inline-flex items-center gap-1 rounded-full bg-sky-100 px-2 py-1 font-semibold text-sky-800"><Timer className="h-3 w-3" />⏱️ {DAYS_LEFT[c.ticketId] ?? 5} Days Left to Bid</span>
              <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-1 font-semibold text-[#1E3A8A]"><Sparkles className="h-3 w-3" />🧠 AI Capability Fit: {c.aiMatch}%</span>
            </div>
            ...
            <div className="mt-3 rounded-lg bg-slate-50 p-2.5 text-xs">
              <p className="flex items-center gap-1 font-semibold text-[#1E3A8A]"><IndianRupee className="h-3.5 w-3.5" /> State Grant {fmt(c.statePoolINR)}</p>
