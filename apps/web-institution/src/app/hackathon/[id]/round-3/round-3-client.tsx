      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="min-w-0 space-y-4">
          <div className="rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
            ...
            <input id="dpr-title" value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1 w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-sm outline-none focus:border-[#2563EB]" />
            <label htmlFor="dpr-scope" className="mt-3 block text-xs font-semibold">Scope + deployment plan</label>
            <textarea id="dpr-scope" value={scope} onChange={(e) => setScope(e.target.value)} rows={3} className="mt-1 w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-sm outline-none focus:border-[#2563EB]" />
            <label htmlFor="dpr-om" className="mt-3 block text-xs font-semibold">O&amp;M handover plan</label>
            <textarea id="dpr-om" value={om} onChange={(e) => setOm(e.target.value)} rows={3} className="mt-1 w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-sm outline-none focus:border-[#2563EB]" />
            <button disabled={!ready} onClick={() => setToast(`DPR submitted for ${ticketId}. BOM within ceiling; defense slot locked.`)} className="mt-4 w-full rounded-lg bg-[#1E3A8A] hover:bg-[#2563EB] px-4 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300 transition-colors">
          ...
          {slotMsg && <p role="status" className="rounded-lg bg-blue-50 border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-900">{slotMsg}</p>}
        </div>
