    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex items-center gap-3">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#1E3A8A] text-sm font-bold text-white">SR</span>
          <div>
      ...
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
        <h3 className="flex items-center gap-1.5 text-sm font-bold"><CalendarCheck className="h-4 w-4 text-[#2563EB]" /> Mentor slot booking</h3>
        <div className="mt-3 grid gap-2">
          {SLOTS.map((s) => (
            <button key={s} onClick={() => { setSlot(s); onBook(s); }} aria-pressed={slot === s} className={`rounded-lg border px-3 py-2 text-left text-sm ${slot === s ? "border-[#1E3A8A] bg-blue-50 text-[#1E3A8A] font-bold" : "border-[#E2E8F0] hover:bg-slate-50"}`}>
              {s}
            </button>
          ))}
        </div>
        {slot && <p role="status" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-800"><CheckCircle2 className="h-3.5 w-3.5 text-[#2563EB]" />Booked: {slot}</p>}
      </div>
    </div>
  );
}
