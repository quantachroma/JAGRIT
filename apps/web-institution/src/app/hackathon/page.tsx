  ...
  return (
    <div className="space-y-4">
      ...
      {openChallenges.map((c, i) => (
        <div key={c.ticketId} className="space-y-3 rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-sky-100 px-2 py-1 text-[11px] font-bold text-[#2563EB]">AI {c.aiMatch}%</span>
          </div>
          ...
          <p className="inline-flex items-center gap-1 text-xs text-slate-500"><Lightbulb className="h-3.5 w-3.5 text-[#2563EB]" /> Tip: open Student R&amp;D Copilot from the sidebar on any arena route.</p>
        </div>
      ))}
    </div>
  );
